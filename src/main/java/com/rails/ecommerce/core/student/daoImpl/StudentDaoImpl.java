...
public class StudentDaoImpl extends GenericDaoImpl<StudentInfo> implements StudentDao {
	...
	@Override
	public PaginationList findAllPage(String cardNo, String cnbh, String jxid, String classSign, 
		...
		if(cnbh != null && !"".equals(cnbh)) {
			if(sql.indexOf("where")==-1) {
				sql = sql + "where student.cnbh = '"+cnbh+"' OR student.stLinkmanp = '"+cnbh+"'";
			} else {
				sql =  sql + " and student.cnbh = '"+cnbh+"' OR student.stLinkmanp = '"+cnbh+"'";
			}
		}
	...
	public PaginationList findAllCheck(String cardNo, String jxid, String classSign, 
		...
		if(usexs != 0) {
			if(sql.indexOf("where")==-1) {
				sql = sql + "where student.usexss <= '"+usexs+"'";
			} else {
				sql = sql + "and student.usexss <= '"+usexs+"'";
			}
		}
